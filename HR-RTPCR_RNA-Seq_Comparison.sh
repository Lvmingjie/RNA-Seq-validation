...
# 2- Blastn the Primers against the DB;
	...
	PathToFastaTranscriptomeFile="/mnt/shared/scratch/pf41880/201610_RTD-barley/All_RNA-Seq-datasetsTogether/5-RNA-Seq_flcDNAs_Iso-Seq-Merge/StringTie-Merge_b1aa-flcDNAs-Iso-Seq_2/StringTie-Merge_b1aa-flcDNAs-Iso-Seq_2-sub/TranscriptsMore30PerCent"
	PathToFastaPrimersFile="/mnt/shared/scratch/pf41880/201610_RTD-barley/16Samples/STAR-Cufflinks-pipeline-deduplicated/RT-PCR_mock-Test/Primers"
	TranscriptomeFileName="Transcripts30.fasta" 
	## The previous fasta file must have the transcript names merged with transcript names -> ">MSTRG.2.1gene=MSTRG.2" not ">MSTRG.2.1 gene=MSTRG.2", so no empty spaces between gene and transcript names in each transcript header (command: sed -r 's/\s+//g' file.fa)
	
	#PrimersFileName="PrimersSequencesWithMLOCs_B1.fasta"
	## PrimersSequencesWithMLOCs_B-clean_C.fasta - version4 : created new list of primers just with the final ones selected by Craig, and "F=" and "R=" were replaced by "C="
	## Primers fasta file (PrimersFileName): the primer names must end with "_R" and "_F" and must form pairs:
	PrimersFileName="PrimersSequencesWithMLOCs_B-clean_C-Plus.fasta" # Just included the primers sequences from the final Craig's selection: Introduced "C="   ## Version5
	PathToSamplesFolder="/mnt/shared/scratch/pf41880/201610_RTD-barley/All_RNA-Seq-datasetsTogether/5-RNA-Seq_flcDNAs_Iso-Seq-Merge/StringTie-Merge_b1aa-flcDNAs-Iso-Seq_2/StringTie-Merge_b1aa-flcDNAs-Iso-Seq_2-sub/TranscriptsMore30PerCent/rtPCR-validation/Salmon/writeMappings" 
	NumberOfReplicates=3
	NameSalmonFiles="quant.sf" ## Value at column 4 (quant.sf file)
	rtPCRInput="rtPCR-Input30August2016-testC.txt"  ## Change the name of this file at the end ## Header: "Primer 	Size	INF2	LEAF	EMBRYO	INF1	NODE"
	...
	###  /home/pf41880/R/ncbi-blast-2.2.28+
	/home/pf41880/R/ncbi-blast-2.2.28+/bin/makeblastdb -in $PathToFastaTranscriptomeFile/$TranscriptomeFileName -out $DataBaseDB -dbtype nucl
	...
	###			
	###			
	/home/pf41880/R/ncbi-blast-2.2.28+/bin/blastn -task blastn-short -num_threads 4 -query $PathToFastaPrimersFile/$PrimersFileName \
...
# 5- Indentify (based on the previous point data) transcripts with perfect matches (both primers) and the problematic cases;

#BlastN_Output="Primers_vs_AllTranscripts.txt"  ## Already reported in the previous script, remove in the end;
...
#rtPCRInput="rtPCR-Input30August2016-test.txt"  ##

		#input files:
		#ListAllDataClusteredPerPrimer_.txt  ##
		#$rtPCR-Input30August2016-test.txt ## Header: "Primer 	Size	INF2	LEAF	EMBRYO	INF1	NODE"
			  ...
			  ############ Create all combinations with both files
				...
				cat Provisory11-RNA-Seq-rtPCRline-Neg.txt Provisory11-RNA-Seq-rtPCRline-Pos.txt|sort -n -k 4,4 >Provisory11-RNA-Seq-rtPCRline-Abs.txt # File with all combinations sorted by the absolute differences of product lengths;
					### Loop Start to choose best pairs
						...
						##### test2
						printf '%s\n' "--333333333 $ProductToExclude1 $ProductToExclude2"
...
######################## To introduce all initial primer (rtPCR) information on the file Best pairs list (add the proportions and the orphan primers (rtPCR data that has no RNA-Seq partner)
		...
		printf '%s\n' "$PrimerCode $ProductSize $PrimerProportions pppppppp "
